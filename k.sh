...
manage_swap() {
    ...
    local desired_size=$((16 * 1024 * 1024))
    ...
    log "INFO" "Creating new 16GB swap file..."
    sudo fallocate -l 16G "$SWAP_FILE"
    if [ $? -ne 0 ]; then
        log "ERROR" "Failed to create 16GB swap file"
        exit 1
    fi
    ...
    log "INFO" "✅ 16GB swap file created and enabled"
}
# Modify run script
modify_run_script() {
